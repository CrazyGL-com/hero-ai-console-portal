...
interface CardSlot {
	...
	tiltA: number;
	tiltB: number;
	scanlineUni: { value: number };
	timeUni: { value: number };
	signature: string;
}
...
function makeTextCardTexture({
	...
	for (let y = 8; y < H; y += 4) ctx.fillRect(8, y, W - 16, 1);
...
export default function PortalStage(props: StageProps) {
	const {
		rootRef,
		size,
		input,
	...
	// Portal
	const portalMeshRef = React.useRef<THREE.Mesh | null>(null);
	const portalMatRef = React.useRef<THREE.ShaderMaterial | null>(null);
	const backglowMeshRef = React.useRef<THREE.Mesh | null>(null);
	const backglowMatRef = React.useRef<THREE.ShaderMaterial | null>(null);

	// Screenshot panel
	const screenGroupRef = React.useRef<THREE.Group | null>(null);
	...
	const screenScanlineUniRef = React.useRef<{ value: number } | null>(null);
	const screenTimeUniRef = React.useRef<{ value: number } | null>(null);
	...
	const cardsRef = React.useRef<CardSlot[]>([]);
	// One-shot cancel flags per slot for in-flight image loads.
	const cardLoadCancelsRef = React.useRef<Array<{ cancelled: boolean } | null>>([]);

	// Particles
	const dustGeomRef = React.useRef<THREE.BufferGeometry | null>(null);
	const dustMatRef = React.useRef<THREE.ShaderMaterial | null>(null);
	const dustPointsRef = React.useRef<THREE.Points | null>(null);
	...
	React.useEffect(() => {
		...
		if (dust) dust.uniforms.u_dpr.value = renderer.getPixelRatio();
	...
	React.useEffect(() => {
		...
		if (bgMesh) bgMesh.scale.set(portalRadius * 3.2, portalRadius * 3.2, 1);
	...
	React.useEffect(() => {
		...
		const img =
			preload && preload.url === screenshot ? preload.img : null;
		if (img) {
		...
		loader.load(
			...
			(tex) => {
				...
				const loaded = tex.image as HTMLImageElement | undefined;
				tex.dispose();
				if (loaded) applyImage(loaded);
			},
			undefined,
			() => {
	...
	React.useEffect(() => {
		...
		const mat = screenMatRef.current;
		if (mat) mat.emissiveIntensity = screenEmissive;
	}, [screenshotX, screenshotY, screenshotScale, screenshotTilt, screenEmissive]);

	// Scanline strength live update
	React.useEffect(() => {
		if (screenScanlineUniRef.current) screenScanlineUniRef.current.value = scanlineStrength;
		for (const c of cardsRef.current) c.scanlineUni.value = scanlineStrength;
	}, [scanlineStrength]);
	...
	const cardConfigsKey = React.useMemo(
		() =>
			cardConfigs
				.map((c) =>
					c.mode === 'image'
	...
	React.useEffect(() => {
		...
		if (cards.length === 0) return; // scene not yet set up
		const cardBaseH = 0.7;

		const applyTextCard = (slot: CardSlot, cfg: CardConfig) => {
		...
		for (let i = 0; i < MAX_CARDS; i++) {
			...
			if (sig === slot.signature) continue;
			slot.signature = sig;
			...
			loader.load(
				...
				(tex) => {
					...
					const img = tex.image as
						...
						| undefined;
					const iw = (img?.naturalWidth || img?.width || 16) as number;
					const ih = (img?.naturalHeight || img?.height || 9) as number;
					...
					cctx.clip();
					if (img) {
						cctx.drawImage(img as CanvasImageSource, 0, 0, cardW, cardH);
					}
					cctx.restore();
	...
	React.useEffect(() => {
		...
		if (geom) geom.setDrawRange(0, Math.max(0, Math.min(MAX_PARTICLES, Math.floor(particleCount))));
	...
	useHeroAnimationFrame(rootRef, ({ delta, elapsed }) => {
		...
		// Portal time uniform.
		const pm = portalMatRef.current;
		if (pm) pm.uniforms.u_time.value = t;
		const bg = backglowMatRef.current;
		if (bg) bg.uniforms.u_time.value = t;

		// Screenshot scanline time.
		if (screenTimeUniRef.current) screenTimeUniRef.current.value = t;
		...
		// Particle uniforms
		const dust = dustMatRef.current;
		if (dust) dust.uniforms.u_time.value = reducedMotion ? elapsed * 0.15 : elapsed;
	...
	return <canvas ref={canvasRef} className="crazygl-acp-canvas" aria-hidden="true" />;
}
